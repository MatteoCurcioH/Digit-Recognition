...
# print(tune_results) # Descomentar si quieres ver la tabla completa

# Extraer los parámetros óptimos
best_cost <- tune_results$best.parameters$cost
best_gamma <- tune_results$best.parameters$gamma
best_model_error <- tune_results$best.performance

cat(paste0("\n--- Parámetros Óptimos Encontrados ---\n"))
cat(paste0("Mejor Cost (C): ", best_cost, "\n"))
cat(paste0("Mejor Gamma: ", best_gamma, "\n"))
cat(paste0("Error de Validación Cruzada (CV): ", round(best_model_error, 4), "\n"))
cat("--------------------------------------\n")
...
save(svm_final_model, file="model_svm.RData")
